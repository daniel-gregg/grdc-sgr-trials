...
class InsecticidesProductsModel(BaseModel):
...
class InsecticidesUnits(str, Enum):
    kilograms = 'kilograms'
    litres = 'litres'
...
class InsecticidesApplicationsModel(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    ...
    def pesticide_product_exists(cls, insecticidename):
        ...
        if sum(insecticideProducts['name'].str.contains(insecticidename))==0:
    ...
    insecticideApplicationTiming: Optional[str] = Field(..., min_length=1, max_length=1000, description="Comment on insecticide timing (optional)")
